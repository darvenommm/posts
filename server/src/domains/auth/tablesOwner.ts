...
export class AuthTablesOwner extends TablesOwner {
  ...
  private async addRolesIfNeed(sql: Sql): Promise<void> {
    ...
    for (const role of roles) {
      ...
      if (Boolean(roleFromDatabase[0])) {
        continue;
      }

      await sql`INSERT INTO posts.roles ${sql({ name: role })};`;
    }
  }

  private async addAdminUserIfNeed(sql: Sql): Promise<void> {
    const admins = await sql`SELECT * FROM posts.users WHERE role = ${Role.ADMIN} LIMIT 1;`;
